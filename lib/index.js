...
//Hacer el eagain con todos los errores
//Hacer el caso onlyBytes, es decir, cuando solo me piden los bytes y
//proporcionan archivos (no datos)

var path = require ("path");
var fs = require ("fs");
var reader = require ("../js/lib/reader");
						...
						fs.readdir (p, function (error, entries){
							...
							if (settings._ignoreIsFunction){
								entries.forEach (function (entry){
									...
									settings._ignoreIsFunction (pa, p, entry,
											function (error, exec){
												if (error) return fail (error);
...
module.exports = function (p, settings, cb){
	...
	settings._onlyBytes = settings.bytes && !settings.chars && !settings.words &&
			!settings.lines && !settings.data;
